...
///
/// The returned object is also a combinator, so that it can be used when using combinators as predicates to match combinators, e.g. using \c HMRConcatenate(HMRBind(), HMRBind()) to destructure a concatenation. It is not useful for parsing, however, only for destructuring pattern matching.
id HMRBind(void);
